...
import com.danger.common.model.AccessLogForm;
import com.danger.common.model.AccessLogModel;
import com.danger.common.service.AccessLogService;
...
public class AccessLogAspect {
    ...
    @Autowired
    private AccessLogService opLogService;
    @Autowired
    ...
    @Pointcut("@annotation(com.danger.common.log.access.AccessLog)")
    ...
    public void after(JoinPoint joinPoint) {
        try {
            AccessLogForm2 ann = getControllerMethodInfo(joinPoint);
    ...
    public static void addLog(String module, String action, String title, HttpServletRequest request, AuthService authService, AccessLogService opLogService){
        AccessLogForm2 map = new AccessLogForm2();
        map.setTitle(title);
    ...
    public static void addLog(AccessLogForm2 logForm, HttpServletRequest request, AuthService authService, AccessLogService opLogService) {
        Long userId = 0L;
        String username = "";
        ...
        AccessLogForm log = new AccessLogForm();
        ...
        ThreadPoolUtil.getPool().execute(new SaveLogThread(log, opLogService));
    }
    ...
    private static class SaveLogThread implements Runnable {
        private AccessLogForm log;
        private AccessLogService logService;

        public SaveLogThread(AccessLogForm esLog, AccessLogService logService) {
            this.log = esLog;
        ...
        @Override
        public void run() {
            AccessLogModel e = BeanParser.obj_to_obj(AccessLogModel.class, log, null);
     ...
     * @throws Exception
     */
    public static AccessLogForm2 getControllerMethodInfo(JoinPoint joinPoint) throws Exception {
        AccessLogForm2 map = new AccessLogForm2();
        map.setTitle("");
        map.setAction("");
        map.setModule("");
        ...
        for (Method method : methods) {
            ...
            title = method.getAnnotation(AccessLog.class).title();
            action = method.getAnnotation(AccessLog.class).action();
            module = method.getAnnotation(AccessLog.class).module();
            map.setTitle(title);
            map.setAction(action);
            map.setModule(module);
        }
